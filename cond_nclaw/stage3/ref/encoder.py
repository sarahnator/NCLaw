...
class TrajEncoder(nn.Module):                                # [E1]
    ...
    def forward(self, F, tau):
        ...
        h = self.phi(x).mean(dim=-2)                          # permutation-invariant pool
        return self.rho(h)
...
def contrastive_loss(za, zb, params_std, temp=0.2, sigma=1.0):   # [E2]
    ...
    lp1 = Fn.log_softmax(logits, dim=-1)
    lp2 = Fn.log_softmax(logits.t(), dim=-1)
...
def train_encoder(encoder, elastic, plastic, F, tau, Fp, tau_scale, params_std,   # [E3]
    ...
    for _ in range(steps):
        ...
        i1, i2 = perm[:K], perm[K:2 * K]                     # two disjoint views
        za = encoder(F[:, i1], tau[:, i1])
        zb = encoder(F[:, i2], tau[:, i2])
        Fr = F[:, i1].reshape(-1, 3, 3)                    # reconstruct the encoded samples
        tr = tau[:, i1].reshape(-1, 3, 3); Fpr = Fp[:, i1].reshape(-1, 3, 3)
        ...
        (loss_recon + lam * loss_con).backward()
        opt.step(); sched.step()
